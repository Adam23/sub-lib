window.GlobalData = flight.component ->
  ...
  @refreshDepth = (data) ->
    ...
    market_depth_ratio = {btccny: 0.1, ethcny: 0.6, ethbtc: 0.6}

    depth_ratio = market_depth_ratio[gon.market.id] || 0.5

    if la && lb
      mid = (_.first(bids)[0] + _.first(asks)[0]) / 2
      offset = Math.min.apply(Math, [(mid-lb[0]) * depth_ratio, (la[0]-mid) * depth_ratio])
    else if !la? && lb
      mid = _.first(bids)[0]
      offset = (mid-lb[0]) * depth_ratio
    else if la && !lb?
      mid = _.first(asks)[0]
      offset = (la[0]-mid) * depth_ratio
