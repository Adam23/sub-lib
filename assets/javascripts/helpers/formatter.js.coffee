class Formatter
  ...
  ticker_price: (price, fillTo = 6) ->
    ...
    if fill = @ticker_fill[fillTo-right.length]
      price = "#{left}.#{right}#{fill}"
    else
      price = "#{left}.#{right.slice(0,fillTo)}"

    price.replace(/0(0+$)/, "0<g>$1</g>")
  ...
  mask_fixed_volume: (volume) ->
    if gon.market.id == 'ethcny'
      @.round(volume, 2).replace(/0(0+$)/, "0<g>$1</g>")
    else
      @.fixAsk(volume).replace(/0(0+$)/, "0<g>$1</g>")
  ...
  amount: (amount, price) ->
    ...
    if gon.market.id == 'ethcny'
      @.round(val, 2).replace(/0(0+$)/, "0<g>$1</g>")
    else
      @.fixAsk(val).replace(/0(0+$)/, "0<g>$1</g>")
  ...
  currency: (value) ->
    if value.match(/\./)
      if value.match(/^0/)
        value.replace(/(^0\.[0]*)(\d*)/, "<span class='float'>$1</span><span class='int'>$2</span>")
      else
        value.replace(/(\d*)\.(\d*)/, "<span class='int'>$1</span><span class='float'>.$2</span>")
    else
