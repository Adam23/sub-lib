@PlaceOrderUI = flight.component ->
  ...
  @checkBand = (price, last) ->
    limit = BigNumber(0.02)
    price.minus(last).absoluteValue().dividedBy(last).greaterThan(limit)
  ...
  @beforeSend = (event, jqXHR) ->
    ...
    if type == 'ask' && last.greaterThan(price) && @checkBand(price, last)
      unless confirm("！警告：您卖出的报价低于市场最新成交价 2% 您确定要以该价格卖出吗？")
        return jqXHR.abort()

    if type == 'bid' && price.greaterThan(last) && @checkBand(price, last)
      unless confirm("！警告：您买入的报价高于市场最新成交价 2% 您确定要以该价格买入吗？")
        return jqXHR.abort()
