@OrderBookUI = flight.component ->
  ...
  @update = (event, data) ->
    @updateOrders(@select('bidBookSel'), _.first(@filter(data.bids), @.attr.bookLimit), 'bid')
    @updateOrders(@select('askBookSel'), _.first(@filter(data.asks), @.attr.bookLimit), 'ask')

  @filter = (orders) ->
    if gon.market.id == "btccny"
      return _.filter orders, (o) ->
        typeof(o[1]) == "string" && o[1].indexOf("0.000") != 0

    if gon.market.id == "ethcny"
      return _.filter orders, (o) ->
        typeof(o[1]) == "string" && o[1].indexOf("0.0") != 0
  ...
  @after 'initialize', ->
    ...
    $('.asks').on 'click', 'tr', (e) =>
      ...
      filted_asks = @filter(gon.asks)
      @placeOrder $('#bid_entry'), _.extend(@computeDeep(e, filted_asks), type: 'ask')
      @placeOrder $('#ask_entry'), {price: BigNumber(filted_asks[i][0]), volume: BigNumber(filted_asks[i][1])}

    $('.bids').on 'click', 'tr', (e) =>
      i = $(e.target).closest('tr').data('order')
      filted_bids = @filter(gon.bids)
      @placeOrder $('#ask_entry'), _.extend(@computeDeep(e, filted_bids), type: 'bid')
      @placeOrder $('#bid_entry'), {price: BigNumber(filted_bids[i][0]), volume: BigNumber(filted_bids[i][1])}
