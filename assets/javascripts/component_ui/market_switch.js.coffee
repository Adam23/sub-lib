window.MarketSwitchUI = flight.component ->
  @attributes
    ...
    marektsFilter: '#show_all_markets'
    marektsRow: 'tbody tr'
    marektsPrimaryRow: 'tbody tr.primary'
  ...
  @after 'initialize', ->
    ...
    @select('marektsRow').hide()
    @select('marektsPrimaryRow').show()

    @select('marektsFilter').on 'click', =>
      @select('marektsRow').show()
